...
- (void)willLoad
{
	...
	[self addName:@"Bad Property Names" jsonString:@"{\"NUMBER one\":1,\"number-two\":2,\"number+3\":3,\"number:four\":4,\"\t\nNumber   \t\tFive  \":5}" expectedResult:DICT(INTNUM(1),@"numberOne",INTNUM(2),@"numberTwo",INTNUM(3),@"number3",INTNUM(4),@"numberFour",INTNUM(5),@"numberFive") options:NDJSONOptionConvertKeysToMedialCapitals];
	[super willLoad];
}
...
- (id)run
{
	...
	id			theResult = [theJSON objectForJSONString:self.jsonString options:self.options error:&theError];
